...
class UserProfilePhoto {
  ...
  static async upsertProfilePhoto(userId, mimeType, photoData) {
    const sql = `
      INSERT INTO user_profile_photos (user_id, mime_type, photo_blob, updated_at)
      VALUES (?, ?, ?, NOW())
  ...
  static async getProfilePhoto(userId) {
    const sql = `
      SELECT mime_type, photo_blob, updated_at
  ...
  static async createDefaultProfilePhoto(userId) {
    ...
    const defaultImagePath = process.env.DEFAULT_PROFILE_IMAGE_PATH || './assets/sample_profile.jpeg';
  ...
  static async createProfilePhotoFromUrl(userId, imageUrl) {
    try {
      ...
      if (!allowedMimeTypes.includes(mimeType)) {
        throw new Error('지원하지 않는 이미지 형식입니다. JPEG, PNG, WebP 형식만 지원합니다.');
      }
      ...
      throw new Error(`프로필 이미지를 다운로드하는 중 오류가 발생했습니다: ${error.message}`);
    }
  }
}

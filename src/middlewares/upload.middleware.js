...
const uploadToVercelBlob = async (file, prefix = "image") => {
  ...
  logger.info(
    `Vercel Blob 업로드 시작: ${file.originalname}, MIME: ${file.mimetype}, Size: ${file.size}, New Filename: ${safeFilename}`
  );
...
const uploadProfileImage = (req, res, next) => {
  singleMulter.fields([{ name: "file", maxCount: 1 }])(
    req,
    res,
    async (err) => {
      ...
      try {
        const existingPhotoUrl = await UserProfilePhoto.getProfilePhotoUrl(
          userId
        );
        ...
        if (
          existingPhotoUrl &&
          existingPhotoUrl.includes("vercel-storage.com")
        ) {
          try {
  ...
  singleMulter.single("file")(req, res, async (err) => {
    ...
    try {
      ...
      return res
        ...
        .json({
          success: false,
          message: `Vercel Blob 업로드 실패: ${uploadError.message}`,
        });
    }
  });
};
...
const uploadReviewPhotos = (req, res, next) => {
  multiMulter.fields([{ name: "photos", maxCount: 3 }])(
    req,
    res,
    async (err) => {
      if (err) return handleUploadError(err, res);
      ...
      if (req.files && req.files.photos) {
        ...
        try {
          const uploadPromises = files.map((file) =>
            uploadToVercelBlob(file, "review_photos")
          );
